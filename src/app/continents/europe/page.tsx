...
const EuropePage = () => {
  ...
  const handleDealSearch = (filter: string) => {
    if (filter === 'flights') {
      window.location.href = `/deals/?type=flight&continent=Europe`;
    } else if (filter === 'hotels') {
      window.location.href = `/deals/?type=hotel&continent=Europe`;
    } else if (filter === 'package') {
      window.location.href = `/deals/?type=package&continent=Europe`;
    } else {
  ...
  const europeDeals = sampleDeals.filter(deal => deal.continent === 'Europe' && deal.type === 'package');

  const packageSets = [];
  for (let i = 0; i < europeDeals.length; i += 2) {
    packageSets.push(europeDeals.slice(i, i + 2));
  }
      ...
      {packageSets.length === 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <p className="text-gray-600 text-lg">
            No package deals currently available for Europe. Check back soon for amazing offers!
          </p>
          <button
            onClick={() => handleDealSearch('Europe')}
            className="mt-4 bg-[#5271FF] text-white font-semibold px-6 py-3 rounded-lg hover:bg-[#4461E8] transition-colors"
          >
            Search All Deals For Europe
          </button>
        </section>
      )}
