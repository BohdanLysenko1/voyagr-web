...
const AustraliaPage = () => {
  ...
  const handleDealSearch = (filter: string) => {
    if (filter === 'flights') {
      window.location.href = `/deals/?type=flight&continent=Australia`;
    } else if (filter === 'hotels') {
      window.location.href = `/deals/?type=hotel&continent=Australia`;
    } else if (filter === 'package') {
      window.location.href = `/deals/?type=package&continent=Australia`;
    } else {
  ...
  const australiaDeals = sampleDeals.filter(deal => deal.continent === 'Australia' && deal.type === 'package');


  const packageSets = [];
  for (let i = 0; i < australiaDeals.length; i += 2) {
    packageSets.push(australiaDeals.slice(i, i + 2));
  }
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Discover the magic of Australia with curated experiences, exclusive deals, and unforgettable adventures across the continent.
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-gray-600">
              <p className="leading-relaxed">
                Book flights across Australia instantly. Unlock exclusive routes and smart search for the best fares.
              </p>
              <p className="leading-relaxed">
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h3 className="text-3xl font-bold text-black">Tailored Australian Packages</h3>
            </div>
            <div className="space-y-4 text-gray-600">
              <p className="leading-relaxed">
              ...
              <p className="leading-relaxed">
                Browse the curated options below to see locations, pricing, and everything included. Find the perfect itinerary or get inspired to customize your own Australian adventure.
              </p>
            </div>
          </div>
        </div>
      ...
      {packageSets.length === 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <p className="text-gray-600 text-lg">
            No package deals currently available for Australia. Check back soon for amazing offers!
          </p>
          <button
            onClick={() => handleDealSearch('Australia')}
            className="mt-4 bg-[#5271FF] text-white font-semibold px-6 py-3 rounded-lg hover:bg-[#4461E8] transition-colors"
          >
            Search All Deals For Australia
          </button>
        </section>
      )}
